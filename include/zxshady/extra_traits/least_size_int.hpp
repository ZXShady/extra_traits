...
namespace tmp {
  ...
  using least_size_uint = std::conditional<
    (N <= UINT_LEAST8_MAX),
    std::uint_least8_t,
    typename std::conditional<
      ...
      typename std::conditional<(N <= UINT_LEAST32_MAX),
                                std::uint_least32_t,
                                typename std::conditional<(N <= UINT_LEAST64_MAX), std::uint_least64_t, void>::type>::type>::type>;

  template<std::int_least64_t N>
  using least_size_int = std::make_signed<typename least_size_uint<static_cast<std::uint_least64_t>((N == INT_LEAST64_MIN) ? INT_LEAST64_MAX
                                                                                                      : (N < 0) ? -N
                                                                                                                : N)>::type>;
  template<std::int_least64_t N>
